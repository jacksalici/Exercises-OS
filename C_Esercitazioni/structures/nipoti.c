...
int main(int argc, char **argv) 
{
	...
	for (i=0; i < N; i++)
	{
		...
		{
			/* codice del figlio */
			printf("Sono il processo figlio di indice %d e pid %d sto per creare il nipote che calcolera' il numero di linee del file %s\n", i, getpid(), argv[i+1]);
			...
			{
			/* codice del nipote */
			printf("Sono il processo nipote del figlio di indice %d e pid %d sto per calcolare il numero di linee del file %s\n", i, getpid(), argv[i+1]);
				...
				/* ogni nipote deve simulare il piping dei comandi nei confronti del nipote e quindi deve chiudere lo standard output e quindi usare la dup sul lato di scrittura della propria pipe */
				close(1);
				dup(p[1]); 			
				/* ogni nipote adesso può chiudere entrambi i lati della pipe: il lato 0 non viene usato e il lato 1 viene usato tramite lo standard ouput */
				close(p[0]);
				close(p[1]);
	...
	*/			
				...
				/* attenzione ai parametri nella esecuzione di wc: aolo -l e terminatore della lista. */ 
...
/* Il padre recupera le informazioni dai figli in ordine di indice */
	for (i=0; i < N; i++)
	{
		...
		printf("Il figlio di indice %d ha convertito il valore %d per il file %s\n", i, valore, argv[i+1]);
